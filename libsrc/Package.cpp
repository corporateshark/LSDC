...
void clPackage::GenerateEnumEditors( const string& FileName )
{
	buffered_stream Out( FileName.c_str() );

	// std include guard
	Out << "/// Autogenerated enum editors for " << FPackageName << " package ///" << endl;
	Out << endl;

	// 1. First the editor functions
	for ( size_t i = 0 ; i < FEnums.size() ; i++ )
	{
		if(FEnums[i].FExported && !FEnums[i].FItems.empty())
		{
			// for C++11 we may use forward defs
			// Out << "enum " << FEnums[i].FEnumName << ";" << endl << endl;
			Out << "#include \"" << FEnums[i].FDeclaredIn << "\"" << endl << endl;

			Out << "BEGIN_ITEM_EDITOR_ENUM(" << FEnums[i].FEnumName << ")" << endl;
			Out << "\tconst char* Items[] = {" << endl;

			for (size_t j = 0 ; j < FEnums[i].FItems.size() ; j++)
			{
				// FEnums[i].FItems[j].FItemName is too long, we strip it from "TypeName_" prefix
				Out << "\t\t\"" << FEnums[i].FItems[j].GetStrippedName(FEnums[i].FEnumName) << "\"";
				if (j < FEnums[i].FItems.size() - 1)
				{
					Out << ", ";
				}
				Out << endl;
			...
			Out << "\t};" << endl;
			Out << "END_ITEM_EDITOR_ENUM(" << FEnums[i].FEnumName << ")" << endl;

			Out << endl;
		}
	}

	// 2. Then the function to select appropriate editor given type name
	Out << "ItemEditorFunc " << FPackageName << "_SelectEnumEditor(const LString& type)" << endl;
	Out << "{" << endl;
	for ( size_t i = 0 ; i < FEnums.size() ; i++ )
	{
		if(FEnums[i].FExported && !FEnums[i].FItems.empty())
		{
			Out << "\tif ( type == \"" << FEnums[i].FEnumName << "\" )" << endl;
			Out << "\t{" << endl;
			Out << "\t\treturn ItemEditor_Enum_" << FEnums[i].FEnumName << ";" << endl;
			Out << "\t}" << endl;
			Out << "\telse" << endl;
		}
	}
	Out << "\t{" << endl << "\t\treturn nullptr;" << endl << "\t}" << endl;
	Out << "}" << endl;
	Out << endl;
}
...
void WriteFileFooter( buffered_stream& OutFile )
{
   ...
   OutFile << g_Indent << g_Indent << "Autogenerated via " << LSDCName << endl;
   OutFile << "*/" << endl;
}
...
/*      if ( !i->second.HasDefaultConstructor() )
      {
         if ( EnableLogging )
         {
            Out << g_Indent << "// Class: " << i->second.FClassName << " has no default constructor - bypassing" << endl;
         }
      }
...
//     cout << "Generating " << CN << endl;
   ...
   for ( int i = 0; i != FPackagesProcsCounter; ++i )
   {
      Out << g_Indent << "RegisterPackage" << FPackageName << "Class" << i << "( Env );" << endl;
   }
   ...
   Out << g_Indent << "// NumClasses : " << RegisteredClasses.size() << endl;
   Out << g_Indent << "Env->Linker->RebuildVirtualTables();" << endl;
   Out << "}" << endl;
...
void clPackage::GenerateEnums()
{
   ...
   for ( size_t i = 0; i != FEnums.size(); i++ )
   {
      ...
      {
         Out << g_Indent << Enum.FItems[j].FItemName;
	...
	GenerateEnumEditors(string( FPackageOutDirectory + string( "/" ) + FPackageName + string( "_EnumEditors.inl" ) ) );
}
